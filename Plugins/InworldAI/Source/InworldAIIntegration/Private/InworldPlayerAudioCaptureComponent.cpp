...
{
	auto* Target = PlayerComponent->GetTargetCharacter();
	if (!Target)
	{
		return;
	}

	const auto& AgentId = Target->GetAgentId();
	if (bEnableAEC)
	{
		...
		InworldSubsystem->SendAudioDataMessageWithAEC(AgentId, Input, Output);
	}
	else
	{
		std::string Input;
		Inworld::Utils::DataArray16ToString(PlayerVoiceCaptureInfo.MicSoundData, Input);
		InworldSubsystem->SendAudioDataMessage(AgentId, Input);
	}
}
